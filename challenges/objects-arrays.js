...
const contactInfo = graduates.map(items => {
  return (`${items.first_name} ${items.email}`); 
})
console.log(contactInfo);

/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called unisWithUni that contains them all. This will be an array of objects. Log the result. */
const unisWithUni = graduates.map(items => {
  return items.university;
})

const filteredUni = unisWithUni.filter(items => {
  return items.includes("Uni");
})
...
zooAnimals.filter(items => {
  if (items.population < 5){
    lowPopulationAnimals.push(items.animal_name);
  }
})
